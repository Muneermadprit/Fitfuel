  ...
  const handleEmailSignIn = async (e) => {
    ...
    try {
      ...
      if (response.data && response.data.status === true) {
        ...
        // ✅ Get Cart API Call
        try {
          const cartResponse = await axios.get('https://api.dailyfit.ae/api/user/get-cart', { withCredentials: true });

          console.log("Cart Data:", cartResponse.data);
          // You can store cart data in localStorage or context if needed
          localStorage.setItem('userCart', JSON.stringify(cartResponse.data));
        } catch (cartError) {
          console.error("Cart Fetch Error:", cartError.response?.data || cartError.message);
          // You can optionally show a non-blocking warning here
        }

        // Navigate to checkout after cart fetch
        navigate('/checkout');
      } else {
        setErrors(prev => ({
          ...prev,
