...
//         <button className="w-full mt-6 bg-[#059033] text-white py-3 rounded-lg font-semibold text-lg hover:bg-purple-700 transition">
//           Continue
//         </button>
//       </div></>
//   );
...
const ProductSummary = () => {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      try {
        const response = await axios.get('https://api.dailyfit.ae/api/user/get-cart',  { withCredentials: true })
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 bg-white shadow-lg rounded-2xl mt-5">
      ...
      {/* Cool Bag Option */}
      <div className="flex items-center mt-4 space-x-3">
        <input type="checkbox" id="coolBag" checked={coolBag} onChange={() => setCoolBag(!coolBag)} className="w-5 h-5 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
        <label htmlFor="coolBag" className="flex items-center space-x-2 cursor-pointer">
          <img src="/api/placeholder/40/40" alt="Cool Bag" className="w-10 h-10 rounded-lg" />
          <span className="text-gray-900 font-medium">Add Cool Bag</span>
        </label>
