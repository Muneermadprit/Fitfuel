...
//   const handleSelection = async (packageId, identifierPakage) => {
//     try {
...
//         const extractedMeals = packages.flatMap(pkg =>
//           pkg.meals ? Object.values(pkg.meals).flatMap(day => day.meals) : []
//         );

//         console.log(extractedMeals, 'kkkkk');

//         // Store in sessionStorage
//         sessionStorage.setItem('mealData', JSON.stringify(extractedMeals));

//         // Update state
//         setMealData(extractedMeals);
//       } else {
...
//   // Retrieve stored meal data from sessionStorage when component loads
//   useEffect(() => {
//     const storedMeals = sessionStorage.getItem('mealData');
//     if (storedMeals) {
//       setMealData(JSON.parse(storedMeals));
//     }
//   }, []);
...
//   // const handleSelection = async (packageId, option, identifierPakage) => {
//   //   try {
//   //     const response = await axios.post('https://api.dailyfit.ae/api/user/get-package-details', {
//   //       packageId,
//   //       identifierPakage
...
//   //       // Extract all dates from meals
//   //       const allDates = packages.flatMap(pkg =>
//   //         pkg.meals ? Object.values(pkg.meals).map(meal => meal.date) : []
//   //       );
...
//               {mealData.map((meal) => {
//                 // const isSelected = mealData[`${selectedDate}-${mealType}`] === meal._id;
...
//                   >
//                     <img
//                       src={meal.image[0]} // Corrected to fetch first image from the array
//                       alt={meal.mealName} // Corrected meal name
//                       className="w-full h-48 object-cover rounded-t-xl"
//                     />
//                     <div className="p-4">
//                       <h4 className="font-medium mb-1">{meal.mealName}</h4>
//                       <p className="text-sm text-gray-600 mb-3">{meal.description}</p>

//                       <div className="text-sm space-y-1 mb-4">
//                         <p>Energy: {meal.moreDetails.energy}</p>
//                         <p>Protein: {meal.moreDetails.protein}</p>
//                         <p>Fat: {meal.moreDetails.fat}</p>
//                         <p>Carbs: {meal.moreDetails.carbohydrates}</p>
//                       </div>

//                       <button
//                         onClick={() => handleMealSelection(selectedDate, mealType, meal._id)}
//                         className={`w-full py-2 rounded-lg font-medium transition-colors 
//             ${isSelected ? 'bg-[#059033] text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
//                       >
...
const MealPlanner = () => {
  ...
  const handleSelection = async (packageId, identifierPackage) => {
    try {
      ...
      alert(packageId ,'The package selected from user')
  ...
  // const handleSelection = async (packageId, planId, identifierPackage) => {
  //   try {
  //     // Update selected state first for immediate UI feedback
  //     setSelectedPackage({
  //       packageId,
  //       planId
  //     });
  ...
  //       console.log(extractedMeals, 'extracted meals');
        ...
        return (
          <div className="min-h-screen bg-gradient-to-b from-purple-50 to-white p-4">
            <div className="max-w-7xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                {packages.map((pkg) => {
                  // For each package, we only get the first package from selectedPlans
                  const packageItem = selectedPlans.packages.length > 0 ? selectedPlans.packages[0] : null;
