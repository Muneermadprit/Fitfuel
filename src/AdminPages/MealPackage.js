...
export default function MealPackage() {
    ...
    const handleSubmit = async (e) => {
        ...
        // Build daily meal assignments with mealName instead of ID
        const dailyMeals = {};

        dates.forEach(date => {
            // Get all meals selected for this date
            dailyMeals[date] = [];

            // Check for each meal if it's selected for this date
            mealData.forEach(meal => {
                const checkboxId = `meal-${date}-${meal._id}`;
                const isChecked = document.getElementById(checkboxId)?.checked;

                if (isChecked) {
                    // Store meal name instead of ID
                    dailyMeals[date].push(meal.mealName);
                }
            });
        });
        ...
        console.log("Daily Meals Assignment:", dailyMeals);
